...
def dwg_to_dxf(dwg: Path, workdir: Path) -> Path:
    ...
    subprocess.run(
        [oda, str(din), str(dout), "ACAD2018", "DXF", "0", "0"],
        capture_output=True,
        text=True,
        timeout=590,
    )
...
def _iter_geometry(entity, depth=0):
    ...
    try:
        if dt == "LINE":
            ...
            c = entity.dxf.center
            circles.append((c[0], c[1], entity.dxf.radius))
        elif dt == "ARC":
            c = entity.dxf.center
            r = entity.dxf.radius
...
def _robust_bounds(segs, circles):
    ...
    return (
        ...
        np.percentile(xs, 99.5),
        np.percentile(ys, 0.5),
        np.percentile(ys, 99.5),
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.dxf:
        ...
        print("--dwg 또는 --dxf 필요", file=sys.stderr)
        return 2
    ...
    segs = _filter_strays(segs, bounds)
    print(
        f"    stray 필터 후 segs={len(segs)}  content_bbox={tuple(round(v) for v in bounds)}",
        flush=True,
    )
    ...
    render(segs, circles, bounds, full, args.dpi)
    outs.append(full)
    if args.frames and args.frames > 1:
        for i, fb in enumerate(split_frames(segs, bounds, args.frames), 1):
            ...
            render(segs, circles, fb, p, args.dpi)
            outs.append(p)
    print("생성:", flush=True)
    for p in outs:
        print(f"    {p}  ({p.stat().st_size:,} bytes)", flush=True)
