...
def reconcile_golden(xlsx: Path, rows: Dict[str, int] = GOLDEN_ROWS_REV3) -> List[dict]:
    ...
    for fl, r in rows.items():
        ...
        m = FloorMeasurement(
            ...
            total_center=P,
            total_side=Q,
            a_side_incl=(G if (G or H) else None),
            a_center=H,
            a_direct=A_h,
            b_side_incl=(J if (J or K) else None),
            b_center=K,
            b_direct=B_h,
        )
        q = compute_floor(m)
...
def extract_zone_measurements(dxf_path: Path) -> List[FloorMeasurement]:  # noqa: ARG001
    ...
    """
    raise NotImplementedError("입력 계층은 권위 출처를 로드하고, DXF/비전 지오메트리는 교차검증으로만 사용하도록 구현한다.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for row in res:
        q = row["engine"]
        A_h, B_h, C_h, R_h, _ = row["golden"]
        allpass &= row["pass"]
        print(
            f"{row['floor']:4s}| {int(q.A):4}/{int(A_h):<4} {int(q.B):3}/{int(B_h):<3} "
            f"{int(q.total):5}/{int(R_h):<5} {int(q.C):5}/{int(C_h):<5}  "
            f"{'PASS' if row['pass'] else 'FAIL'}"
        )
    print(f"\n회계모델 검증: {'ALL PASS ✓' if allpass else 'FAIL ✗'}  " f"(반쪽 측정값→대칭확장→C잔여 가 골든과 일치)")
