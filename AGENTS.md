...
- 이 파일은 현재 **13,694줄**짜리 monolith이며 V2(`DrawingCompareWorkbenchV2` 클래스)가 **유일한** 메인 윈도우다.
...
- 2026-06-16: god-object 분해 **1차 위성 추출** — 순수 overlay 헬퍼 6종 + 상수를 `src/gui/workbench_overlay_model.py`로 이동(-142줄), monolith는 re-import로 공개 API 보존. 후속 시퀀스: `docs/MONO_DECOMPOSITION_PLAN.md`. 동일 re-export 패턴으로 각 추출은 net-negative 유지.
