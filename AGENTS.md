...
- 이 파일은 현재 **13,199줄**짜리 monolith이며 V2(`DrawingCompareWorkbenchV2` 클래스)가 **유일한** 메인 윈도우다. (줄 수는 항상 `wc -l` 기준으로 적는다 — Python `len(splitlines())` 기반 측정은 다중 줄을 단일 리스트 원소로 넣으면 과소계상된다.)
