...
- 이 파일은 현재 **13,557줄**짜리 monolith이며 V2(`DrawingCompareWorkbenchV2` 클래스)가 **유일한** 메인 윈도우다.
...
- 2026-06-16: god-object 분해 위성 추출 — **1차** 순수 overlay 헬퍼 6종+상수→`workbench_overlay_model.py`(-142), **2차** 순수 요약/포맷 헬퍼 7종(`natural_change_summary`·`format_top_issue_label`·`format_pattern_group_label`·`_int_value`·`_format_count`·`_change_grade`·`_ko_change_type`)→`workbench_summary_format.py`(-137). monolith는 re-import로 공개 API 보존. 후속 시퀀스: `docs/MONO_DECOMPOSITION_PLAN.md`. 동일 re-export 패턴으로 각 추출은 net-negative 유지.
